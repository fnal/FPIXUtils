...
int eff( string newmod, string fileDesg ){
	...
	std::string phRunFileName( "phrun" );
		...
		if (highRateFound) {
			...
			for (int iRoc=0;iRoc<nRocs;iRoc++) {
				...
				if( i < ( len-2) ){
					...
					if( inst == "KU" ) { sprintf(xraymapName, "Xray/hMap_Ag_C%d_V0;1", iRoc);}
					else { if( i == low ) { sprintf(xraymapName, "Xray/hMap_DCLowRate_C%d_V0", iRoc); } else { sprintf(xraymapName, "Xray/hMap_DCHighRate_C%d_V0", iRoc); }}
                                        curTfile.GetObject(xraymapName, xraymap);
                                        if (xraymap == 0) {
                                                std::cout << "ERROR: x-ray hitmap not found!" << std::endl;
                                        }
